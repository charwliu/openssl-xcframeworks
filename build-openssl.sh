...
#
#  - Supports OpenSSL-3.1.2 and newer. It might work with version 1.1.0, but testing begins with
...
#
#  Changes by Charlie LIU during Oct. 2023.
#  Copyright 2023 Charwlie LIU. All right reserved.
#
...
# of defaults.
TARGETS_AVAILABLE="iossimulator-arm64 iossimulator-x86_64 ios64-cross-arm64 mac-catalyst-arm64 mac-catalyst-x86_64 tvsimulator-arm64 tvsimulator-x86_64 tvos64-cross-arm64 macos64-x86_64 macos64-arm64 watchos-cross-armv7k watchos-cross-arm64_32 watchos-cross-arm64 watchsimulator-x86_64 watchsimulator-arm64"
...
CONFIG_OPTIONS="${CONFIG_OPTIONS:-'no-ssl3 no-engine'}"
...
if [ ${#LIBSSL_MACOS[@]} -gt 0 ]; then
  ...
  echo " =====>macOS SSL and Crypto lib files:"
...
if [ ${#LIBSSL_CATALYST[@]} -gt 0 ]; then
  ...
  echo "  =====>Catalyst SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-Catalyst.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-Catalyst.a"
fi

# Build iOS library if selected for build
if [ ${#LIBSSL_IPHONEOS[@]} -gt 0 ]; then
  echo "Build library for iOS..."
  lipo -create ${LIBSSL_IPHONEOS[@]} -output "${CURRENTPATH}/lib/libssl-iPhoneOS.a"
  lipo -create ${LIBCRYPTO_IPHONEOS[@]} -output "${CURRENTPATH}/lib/libcrypto-iPhoneOS.a"
  echo "  =====>iOS SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-iOS-Sim.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-iOS-Sim.a"
fi

# Build iOS simulators library if selected for build
if [ ${#LIBSSL_IPHONESIM[@]} -gt 0 ]; then
  echo "Build library for iOS..."
  lipo -create ${LIBSSL_IPHONESIM[@]} -output "${CURRENTPATH}/lib/libssl-iPhoneSimulator.a"
  lipo -create ${LIBCRYPTO_IPHONESIM[@]} -output "${CURRENTPATH}/lib/libcrypto-iPhoneSimulator.a"
  echo "  =====>iOS Simulator SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-iPhoneSimulator.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-iPhoneSimulator.a"
fi

# Build tvOS library if selected for build
if [ ${#LIBSSL_TVOS[@]} -gt 0 ]; then
  echo "Build library for AppleTVOS..."
  lipo -create ${LIBSSL_TVOS[@]} -output "${CURRENTPATH}/lib/libssl-appleTVOS.a"
  lipo -create ${LIBCRYPTO_TVOS[@]} -output "${CURRENTPATH}/lib/libcrypto-appleTVOS.a"
  echo "  =====>AppleTVOS SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-appleTVOS.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-appleTVOS.a"
fi

# Build tvOS simulators library if selected for build
if [ ${#LIBSSL_TVSIM[@]} -gt 0 ]; then
  echo "Build library for AppleTV Simulator..."
  lipo -create ${LIBSSL_TVSIM[@]} -output "${CURRENTPATH}/lib/libssl-appleTVSimulator.a"
  lipo -create ${LIBCRYPTO_TVSIM[@]} -output "${CURRENTPATH}/lib/libcrypto-appleTVSimulator.a"
  echo "  =====>AppleTV Simulator SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-appleTVSimulator.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-appleTVSimulator.a"
fi

# Build watchOS library if selected for build
if [ ${#LIBSSL_WATCHOS[@]} -gt 0 ]; then
  ...
  echo "  =====>watchOS SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-watchOS.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-watchOS.a"
fi

# Build watchOS library if selected for build
if [ ${#LIBSSL_WATCHSIM[@]} -gt 0 ]; then
  echo "Build library for Watch Simulator..."
  lipo -create ${LIBSSL_WATCHSIM[@]} -output "${CURRENTPATH}/lib/libssl-WatchSimulator.a"
  lipo -create ${LIBCRYPTO_WATCHSIM[@]} -output "${CURRENTPATH}/lib/libcrypto-WatchSimulator.a"
  echo "  =====>Watch Simulator SSL and Crypto lib files:"
  echo "    ${CURRENTPATH}/lib/libssl-WatchSimulator.a"
  echo "    ${CURRENTPATH}/lib/libcrypto-WatchSimulator.a"
fi
  ...
  for OPENSSLCONF_CURRENT in "${OPENSSLCONF_ALL[@]}" ; do
    ...
    case "${OPENSSLCONF_CURRENT}" in
      ...
      *_watchos_arm64_32.h)
      ...
      ;;
      *_watchos_arm64.h)
        DEFINE_CONDITION="TARGET_OS_WATCHOS && TARGET_OS_EMBEDDED && TARGET_CPU_ARM64"
      ;;
