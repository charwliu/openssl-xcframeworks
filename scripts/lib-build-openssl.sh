...
run_make()
{
  ...
  if [ "${LOG_VERBOSE}" == "verbose" ]; then
    make -j "${BUILD_THREADS}" build_libs | tee -a "${LOG}"
  else
    (make -j "${BUILD_THREADS}" build_libs >> "${LOG}" 2>&1) & spinner
  fi
...
finish_build_loop()
{
  ...
  if [[ "${PLATFORM}" == AppleTVOS* ]]; then
    ...
    LIBSSL_TVSIM+=("${TARGETDIR}/lib/libssl.a")
    LIBCRYPTO_TVSIM+=("${TARGETDIR}/lib/libcrypto.a")
    OPENSSLCONF_SUFFIX="tvsimulator_${ARCH}"
    ...
    LIBSSL_WATCHSIM+=("${TARGETDIR}/lib/libssl.a")
    LIBCRYPTO_WATCHSIM+=("${TARGETDIR}/lib/libcrypto.a")
    OPENSSLCONF_SUFFIX="watchsimulator_${ARCH}"
  elif [[ "${PLATFORM}" == iPhoneOS* ]]; then
    LIBSSL_IPHONEOS+=("${TARGETDIR}/lib/libssl.a")
    LIBCRYPTO_IPHONEOS+=("${TARGETDIR}/lib/libcrypto.a")
    OPENSSLCONF_SUFFIX="iphoneos_${ARCH}"
  elif [[ "${PLATFORM}" == iPhoneSimulator* ]]; then
    LIBSSL_IPHONESIM+=("${TARGETDIR}/lib/libssl.a")
    LIBCRYPTO_IPHONESIM+=("${TARGETDIR}/lib/libcrypto.a")
    OPENSSLCONF_SUFFIX="iphonesimulator_${ARCH}"
...
target_build_loop()
{
    for TARGET in $TARGETS; do
      ...
      elif [[ "${TARGET}" == "tvsimulator-"* ]]; then
        PLATFORM="AppleTVSimulator"
      elif [[ "${TARGET}" == "tvos64-cross-"* ]]; then
        PLATFORM="AppleTVOS"
      elif [[ "${TARGET}" == "macos"* ]]; then
        PLATFORM="MacOSX"
      elif [[ "${TARGET}" == "watchsimulator-"* ]]; then
